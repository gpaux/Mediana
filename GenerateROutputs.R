
library(Mediana)
setwd("C:/Users/gauti/CloudStation/Mediana Package/R/Package_v1.0.6/Mediana-gh-pages")

rm(list = ls())
source("Case study 1 (normally distributed endpoint).R")
rm(list = ls())
source("Case study 1 (binary endpoint).R")
rm(list = ls())
source("Case study 1 (survival-type endpoint).R")
rm(list = ls())
source("Case study 1 (survival-type endpoint with censoring).R")
rm(list = ls())
source("Case study 1 (count-type endpoint).R")
rm(list = ls())
source("Case study 2.R")
rm(list = ls())
source("Case study 3.R")
rm(list = ls())
source("Case study 4.R")
rm(list = ls())
source("Case study 5.R")
rm(list = ls())
source("Case study 6.R")

rm(list = ls())
source("mult-cs1-mediana.R")
rm(list = ls())
source("mult-cs2-mediana.R")
rm(list = ls())
source("mult-cs3-mediana.R")
rm(list = ls())
source("subgroup-cs1-mediana.R")
rm(list = ls())
source("subgroup-cs2-mediana.R")
rm(list = ls())
source("subgroup-cs3-mediana.R")